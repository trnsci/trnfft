...
if HAS_NKI:
    ...
    #
    # Design revision (v2): twiddle multiply moved to the driver (PyTorch ops
    # on XLA device), kernel receives pre-twiddled input and applies W_8 only.
    #
    # Reason: nl.load_transpose2d requires an external HBM function-argument
    # tensor as source. A kernel-local nl.ndarray(buffer=nl.shared_hbm) scratch
    # buffer is NOT addressable by load_transpose2d in NKI 0.3.0 — this caused
    # the original two-phase kernel to fail NEFF compilation on hardware.
    #
    # Twiddle multiply in PyTorch is a single element-wise op on the XLA device:
    # fast (similar overhead to what radix-4 already incurs for pack/unpack).
    #
    # Input/output layout
    # --------------------
    # - a_re, a_im : (total_groups, 8) HBM — pre-twiddled input (external arg)
    ...
    def stockham_radix8_w8_kernel(a_re, a_im, w8_re, w8_im):
        """W_8 DFT matmul via Tensor-engine nc_matmul.

        Input ``a_re, a_im`` are ``(total_groups, 8)`` pre-twiddled tensors;
        twiddle multiply is done by the driver before this call.  ``w8_re,
        w8_im`` are the ``(8, 8)`` W_8 DFT matrix (shared across all groups).

        Uses nl.load_transpose2d on function-argument HBM tensors (a_re/a_im)
        — NKI 0.3.0 supports this; kernel-local scratch HBM does not work.
        """
        total_groups, _ = a_re.shape

        out_re = nl.ndarray((total_groups, 8), dtype=a_re.dtype, buffer=nl.shared_hbm)
        out_im = nl.ndarray((total_groups, 8), dtype=a_im.dtype, buffer=nl.shared_hbm)
        ...
        # W_8 is shared across all groups; load once outside the tile loop.
        ...
        for m in nl.affine_range(n_partition_tiles):
            ...
            # Load pre-twiddled input with transpose: (groups_chunk, K=8) → (K=8, groups_chunk)
            # nl.load_transpose2d works on external HBM function arguments.
            ar_t = nl.load_transpose2d(a_re[m_off : m_off + groups_chunk, :TILE_K])
            ai_t = nl.load_transpose2d(a_im[m_off : m_off + groups_chunk, :TILE_K])
            ...
            if a_re.dtype != nl.float32:
                cr_sbuf = nl.cast(cr_sbuf, dtype=a_re.dtype)
                ci_sbuf = nl.cast(ci_sbuf, dtype=a_re.dtype)
    ...
    stockham_radix8_stage_kernel = stockham_radix8_w8_kernel

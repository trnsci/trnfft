# Terraform — trnfft Neuron CI Instance
...
user-data to finish (installs Neuron SDK, clones trnfft), then stop it:
...
| `instance_tag` | `trnfft-ci-trn1` | Must match `scripts/run_neuron_tests.sh [type]` arg |
| `vpc_id` | (required) | |
